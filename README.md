...
> ## ⚠️ v2.0.0 임박 — tuple-form Icon API 제거 예정
>
> `<Icon iconType={['system', 'add']}>` / `<Button leadIcon={[...]}>` / `<CellIcon iconType={[...]}>` 등 **tuple 형식**의 모든 아이콘 API가 v2.0.0에서 제거됩니다. 마이그레이션 안 한 코드는 v2.0에서 빌드 실패 (타입 에러).
>
> **이유:** tuple 경로가 consumer 빌드에 ~2.5MB async chunk를 만듭니다. Component-ref API로 바꾸면 청크가 사라집니다.
>
> **지금 할 일:**
> ```bash
> npx blumnai-icon-codemod ./src   # 자동 마이그레이션 (static literal tuple)
> ```
> 또는 v2.0 출시 후 escape hatch: `import from '@blumnai-studio/blumnai-design-system/icons/icon-legacy'`
>
> 자세한 내용: [MIGRATION.md](./MIGRATION.md#v200--icon-tuple-form-제거)
>
