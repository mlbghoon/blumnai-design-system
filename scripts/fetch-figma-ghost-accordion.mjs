...
/**
 * Fetch Ghost style AccordionItem variants from Figma
 * 
...
const NODE_IDS = [
  { id: '3892:9724', name: 'Style=Ghost, State=Default' },
  { id: '3892:9727', name: 'Style=Ghost, State=Hover' },
  { id: '3892:9721', name: 'Style=Ghost, State=Opened' },
];
const OUT_PATH = path.join(ROOT, 'src', 'components', 'accordion', 'AccordionItem', 'source', 'ghost-style-variants.json');
...
const main = async () => {
  ...
  console.log(`Fetching Ghost style AccordionItem variants from Figma...`);
  ...
  try {
    ...
    const output = {
      metadata: {
        ...
        description: 'Ghost style AccordionItem variants (Default, Hover, Opened states)',
        source: 'Figma API',
      },
      variants,
    };
    ...
    console.log(`\n✅ Ghost style variants written to: ${OUT_PATH}`);
    console.log(`   - Found ${variants.length} variants`);
    variants.forEach(v => {
      console.log(`     • ${v.name} (${v.nodeId})`);
    });
  } catch (error) {
    console.error('❌ Error fetching Ghost style variants:', error.message);
    process.exit(1);
  }
};
