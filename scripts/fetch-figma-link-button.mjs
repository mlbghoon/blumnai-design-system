...
/**
 * Fetch LinkButton component data from Figma
 * 
 * This script fetches the LinkButton component structure and styles from Figma
...
const DEFAULT_NODE_ID = '2097:2376';
const OUT_PATH = path.join(ROOT, 'src', 'components', 'button', 'LinkButton', 'source', 'figma-data.json');
...
const main = async () => {
  ...
  console.log(`Fetching LinkButton data from Figma...`);
  ...
  try {
    ...
    const linkButtonData = {
      ...
      structure: {
        description: 'LinkButton component structure from Figma',
        node: {
          id: nodeDoc.id,
          name: nodeDoc.name,
          type: nodeDoc.type,
      ...
      variants: {
        description: 'LinkButton variants (sizes, states)',
      },
    };
    ...
    if (nodeDoc.children) {
      ...
      linkButtonData.variants.items = variants;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, JSON.stringify(linkButtonData, null, 2) + '\n');
    
    console.log(`\n✅ LinkButton data written to: ${outPath}`);
    console.log(`   - Found ${linkButtonData.variants.items?.length || 0} variants`);
  } catch (error) {
    console.error('❌ Error fetching LinkButton data:', error.message);
    process.exit(1);
  }
};
