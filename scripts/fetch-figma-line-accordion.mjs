...
/**
 * Fetch Line style AccordionItem variants from Figma
 * 
...
const NODE_IDS = [
  { id: '3892:9725', name: 'Style=Line, State=Default' },
  { id: '3892:9723', name: 'Style=Line, State=Hover' },
  { id: '3892:9719', name: 'Style=Line, State=Opened' },
];
const OUT_PATH = path.join(ROOT, 'src', 'components', 'accordion', 'AccordionItem', 'source', 'line-style-variants.json');
...
const main = async () => {
  ...
  console.log(`Fetching Line style AccordionItem variants from Figma...`);
  ...
  try {
    ...
    const output = {
      metadata: {
        ...
        description: 'Line style AccordionItem variants (Default, Hover, Opened states)',
        source: 'Figma API',
      },
      variants,
    };
    ...
    console.log(`\n✅ Line style variants written to: ${OUT_PATH}`);
    console.log(`   - Found ${variants.length} variants`);
    variants.forEach(v => {
      console.log(`     • ${v.name} (${v.nodeId})`);
    });
  } catch (error) {
    console.error('❌ Error fetching Line style variants:', error.message);
    process.exit(1);
  }
};
