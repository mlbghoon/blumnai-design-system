...
/**
 * Fetch Default style AccordionItem variants from Figma
 * 
 * Fetches three states: Default, Hover, Opened
 */
...
const NODE_IDS = [
  { id: '3892:9722', name: 'Style=Default, State=Default' },
  { id: '3892:9728', name: 'Style=Default, State=Hover' },
  { id: '3892:9730', name: 'Style=Default, State=Opened' },
];
const OUT_PATH = path.join(ROOT, 'src', 'components', 'accordion', 'AccordionItem', 'source', 'default-style-variants.json');
...
const main = async () => {
  ...
  console.log(`Fetching Default style AccordionItem variants from Figma...`);
  ...
  try {
    ...
    const output = {
      metadata: {
        ...
        description: 'Default style AccordionItem variants (Default, Hover, Opened states)',
        source: 'Figma API',
      },
      variants,
    };
    ...
    console.log(`\n✅ Default style variants written to: ${OUT_PATH}`);
    console.log(`   - Found ${variants.length} variants`);
    variants.forEach(v => {
      console.log(`     • ${v.name} (${v.nodeId})`);
    });
  } catch (error) {
    console.error('❌ Error fetching Default style variants:', error.message);
    process.exit(1);
  }
};
