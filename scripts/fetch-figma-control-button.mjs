...
/**
 * Fetch ControlButton component data from Figma
 * 
 * This script fetches the ControlButton component structure and styles from Figma
...
const DEFAULT_NODE_ID = '3852:15208';
const OUT_PATH = path.join(ROOT, 'src', 'components', 'button', 'ControlButton', 'source', 'figma-data.json');
...
const main = async () => {
  ...
  console.log(`Fetching ControlButton data from Figma...`);
  ...
  try {
    ...
    const controlButtonData = {
      ...
      structure: {
        description: 'ControlButton component structure from Figma',
        node: {
          id: nodeDoc.id,
          name: nodeDoc.name,
          type: nodeDoc.type,
      ...
      variants: {
        description: 'ControlButton variants (sizes, shapes, states)',
      },
    };
    ...
    if (nodeDoc.children) {
      ...
      controlButtonData.variants.items = variants;
    }

    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, JSON.stringify(controlButtonData, null, 2) + '\n');
    
    console.log(`\n✅ ControlButton data written to: ${outPath}`);
    console.log(`   - Found ${controlButtonData.variants.items?.length || 0} variants`);
  } catch (error) {
    console.error('❌ Error fetching ControlButton data:', error.message);
    process.exit(1);
  }
};
