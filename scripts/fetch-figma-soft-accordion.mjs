...
/**
 * Fetch Soft style AccordionItem variants from Figma
 * 
...
const NODE_IDS = [
  { id: '3892:9720', name: 'Style=Soft, State=Default' },
  { id: '3892:9726', name: 'Style=Soft, State=Hover' },
  { id: '3892:9729', name: 'Style=Soft, State=Opened' },
];
const OUT_PATH = path.join(ROOT, 'src', 'components', 'accordion', 'AccordionItem', 'source', 'soft-style-variants.json');
...
const main = async () => {
  ...
  console.log(`Fetching Soft style AccordionItem variants from Figma...`);
  ...
  try {
    ...
    const output = {
      metadata: {
        ...
        description: 'Soft style AccordionItem variants (Default, Hover, Opened states)',
        source: 'Figma API',
      },
      variants,
    };
    ...
    console.log(`\n✅ Soft style variants written to: ${OUT_PATH}`);
    console.log(`   - Found ${variants.length} variants`);
    variants.forEach(v => {
      console.log(`     • ${v.name} (${v.nodeId})`);
    });
  } catch (error) {
    console.error('❌ Error fetching Soft style variants:', error.message);
    process.exit(1);
  }
};
