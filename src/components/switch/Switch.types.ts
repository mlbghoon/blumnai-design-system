...
export interface SwitchProps
   ...
   * @example
   * ```tsx
   * import { Switch, Icon, RiCheckLine, RiCloseLine } from '@blumnai-studio/blumnai-design-system';
   *
   * // 고정 아이콘
   * <Switch thumbIcon={<Icon icon={RiCheckLine} size={10} />} />
   * // 상태별 아이콘
   * <Switch thumbIcon={(checked) => <Icon icon={checked ? RiCheckLine : RiCloseLine} size={10} />} />
   * ```
   */
