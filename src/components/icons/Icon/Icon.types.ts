...
/**
 * 아이콘 색상 토큰
 * src/styles/tokens/colors.css의 --icon-* 변수 기반
 */
...
/**
 * 아이콘 색상 - 토큰 또는 CSS 색상값
 * - 토큰: 'default', 'destructive' 등 -> var(--icon-token)으로 변환
 * - CSS 색상: '#fff', 'rgb()', 'rgba()' 등 -> 그대로 사용
 */
...
export type DevelopmentIconName =
  | 'braces'
  | 'brackets'
  | 'bug'
  | 'bug-2'
...
export type IconType =
  ...
  | ['buildings', BuildingsIconName]
  | ['business', BusinessIconName]
  | ['communication', CommunicationIconName]
  | ['design', DesignIconName]
  | ['development', DevelopmentIconName]
  | ['device', DeviceIconName]
  | ['document', DocumentIconName]
  | ['editor', EditorIconName]
  | ['finance', FinanceIconName]
  | ['food', FoodIconName]
  | ['health', HealthIconName]
  | ['map', MapIconName]
  | ['media', MediaIconName]
  | ['others', OthersIconName]
...
export interface SharedIconProps extends Omit<SVGProps<SVGSVGElement>, 'children' | 'cursor'> {
  ...
  /**
   * 아이콘 색상 토큰
   * 토큰은 자동으로 var(--icon-{token}) CSS 변수로 변환됩니다
   * @example color="default" -> var(--icon-default)
   */
  color?: IconColor;
  /**
   * 추가 CSS 클래스명
