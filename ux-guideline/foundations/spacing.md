...
> **매핑 원칙:** DS spacing은 숫자가 곧 px 값이다. 같은 숫자의 DS 유틸리티가 있으면 그대로 사용하고, 없으면 가장 가까운 값으로 억지 변환하지 말고 Tailwind 값을 유지한다.
...
> 예: `p-4` → `padding-4`, `px-8` → `padding-x-8`, `py-10` → `padding-y-10`, `pt-8` → DS 미지원이므로 Tailwind 유지
