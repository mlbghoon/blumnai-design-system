...
- divider는 **기본값(lg) 우선**. spacing 선택 기준은 §7 참조.
...
> **DS DataGrid 컴포넌트를 적용한다.** 상세 규칙은 [table.md](table.md) 참조.
...
Divider의 `spacing` prop은 구분 대상의 관계로 결정한다.

| spacing | 값 | 기준 | 예시 |
|---------|-----|------|------|
| `sm` | 8px | divider가 나누는 양쪽이 **같은 컴포넌트의 내부 요소**일 때 | 툴바 내 버튼 그룹 간, 드롭다운 메뉴 카테고리 간 |
| `md` | 12px | **서로 다른 컴포넌트**이지만 하나의 영역에 속할 때 | 필터 조건부 ↔ 키워드 검색부, 폼 내 필드 그룹 간 |
| `lg` | 16px | **카드/패널 내부**에서 독립된 영역 간 구분 (기본값) | 카드 내 상단 정보 ↔ 하단 액션 영역 |
| `xl` | 24px | **섹션 그룹** 간 구분 | 페이지 헤더 ↔ 본문 |

### AI 판단 규칙

> **divider가 나누는 양쪽이 같은 컴포넌트의 props/children이면 `sm` 이하, 다른 컴포넌트끼리면 `md` 이상 우선 사용.**

```
판단 순서:
1. 양쪽이 같은 컴포넌트 내부인가? → sm 이하
2. 같은 영역(컨테이너) 안의 다른 컴포넌트인가? → md 이상
3. 카드/패널 내부의 독립 영역인가? → lg (기본값)
4. 판단이 안 되면 → lg (기본값 유지, 임의로 축소 금지)
```

### 방향별 적용

| 방향 | spacing 적용 축 | 사용 맥락 |
|------|-----------------|-----------|
| `horizontal` (기본) | 상하 padding | 수평선으로 위아래 영역 구분 |
| `vertical` | 좌우 padding | 수직선으로 좌우 요소 구분 (툴바, 인라인 그룹 등) |
